...
#include "RemoteCameraOutputSession.h"
#include "Channels/RemoteSessionInputChannel.h"
#include "Widgets/SVirtualWindow.h"
#include "GameFramework/PlayerController.h"

namespace RemoteCameraOutputSession
{
	static const FName LevelEditorName(TEXT("LevelEditor"));
	//static const FSoftClassPath EmptyUMGSoftClassPath(TEXT("WidgetBlueprint'/RemoteCamera/Assets/GUI/EmptyUMG.EmptyUMG_C'"));
}

void URemoteCameraOutputSession::Initialize()
{
...
void URemoteCameraOutputSession::Deinitialize()
{
	MediaOutput = nullptr;
...
void URemoteCameraOutputSession::Activate()
{
...
void URemoteCameraOutputSession::Deactivate()
{
...
void URemoteCameraOutputSession::Tick(const float DeltaTime)
{
...
void URemoteCameraOutputSession::CreateRemoteSession()
{
	if (!bInitialized)
	{
		UE_LOG(LogRemoteCamera, Warning, TEXT("CreateRemoteSession has been called, but has not been initialized yet"));
		return;
	}
	...
	if (!RemoteSessionHost.IsValid())
	{
		if (IRemoteSessionModule* RemoteSession = FModuleManager::LoadModulePtr<IRemoteSessionModule>("RemoteSession"))
		{
			...
			if (RemoteSessionHost.IsValid())
			{
				RemoteSessionHost->RegisterChannelChangeDelegate(FOnRemoteSessionChannelChange::FDelegate::CreateUObject(this, &URemoteCameraOutputSession::OnRemoteSessionChannelChange));
			...
			{
				UE_LOG(LogRemoteCamera, Error, TEXT("Unable to create a RemoteSession host on port %d. Check the port is not in use."), PortNumber);
			}
		}
	}
	else
	{
		UE_LOG(LogRemoteCamera, Warning, TEXT("CreateRemoteSession has been called, but there is already a valid RemoteSessionHost!"));
	}
}

void URemoteCameraOutputSession::DestroyRemoteSession()
{
	if (RemoteSessionHost.IsValid())
	{
...
void URemoteCameraOutputSession::OnRemoteSessionChannelChange(IRemoteSessionRole* Role, TWeakPtr<IRemoteSessionChannel> Channel, ERemoteSessionChannelChange Change)
{
...
void URemoteCameraOutputSession::OnImageChannelCreated(TWeakPtr<IRemoteSessionChannel> Instance)
{
	...
	if (ImageChannel)
	{
		...
		if (UVCamOutputComposure* ComposureProvider = Cast<UVCamOutputComposure>(GetOtherOutputProviderByIndex(FromComposureOutputProviderIndex)))
		{
			if (ComposureProvider->FinalOutputRenderTarget)
			{
				...
				UE_LOG(LogRemoteCamera, Log, TEXT("ImageChannel callback - MediaCapture set with ComposureRenderTarget"));
			}
			else
			{
				UE_LOG(LogRemoteCamera, Warning, TEXT("ImageChannel callback - Composure usage was requested, but the specified ComposureOutputProvider has no FinalOutputRenderTarget set"));
			}
		}
		else
		{
			...
			if (TSharedPtr<FSceneViewport> PinnedSceneViewport = SceneViewport.Pin())
			{
				...
				UE_LOG(LogRemoteCamera, Log, TEXT("ImageChannel callback - MediaCapture set with viewport"));
			}
		}
		*/
		TWeakPtr<FSceneViewport> SceneViewport = GetTargetSceneViewport();
...
void URemoteCameraOutputSession::OnInputChannelCreated(TWeakPtr<IRemoteSessionChannel> Instance)
{
		...
		if (UMGClass && UMGWidget)
		{
			...
			if (UVCamOutputComposure* ComposureProvider = Cast<UVCamOutputComposure>(GetOtherOutputProviderByIndex(FromComposureOutputProviderIndex)))
			{
				if (UVPFullScreenUserWidget* ComposureUMGWidget = ComposureProvider->GetUMGWidget())
				{
					...
					UE_LOG(LogRemoteCamera, Log, TEXT("InputChannel callback - Routing input to active viewport with Composure UMG"));
				}
				else
				{
					UE_LOG(LogRemoteCamera, Warning, TEXT("InputChannel callback - Composure usage was requested, but the specified ComposureOutputProvider has no UMG set"));
				}
			}
			else
			{
				InputWindow = UMGWidget->PostProcessDisplayType.GetSlateWindow();
				UE_LOG(LogRemoteCamera, Log, TEXT("InputChannel callback - Routing input to active viewport with UMG"));
			}
			*/
			InputWindow = UMGWidget->PostProcessDisplayType.GetSlateWindow();
			UE_LOG(LogRemoteCamera, Log, TEXT("InputChannel callback - Routing input to active viewport with UMG"));
			...
			InputChannel->GetOnRouteTouchDownToWidgetFailedDelegate()->AddUObject(this, &URemoteCameraOutputSession::OnTouchEventOutsideUMG);
		}
		else
		{
			...
			UE_LOG(LogRemoteCamera, Log, TEXT("InputChannel callback - Routing input to active viewport"));
		}
	}
}

void URemoteCameraOutputSession::OnTouchEventOutsideUMG(const FVector2D& InViewportPosition)
{
...
bool URemoteCameraOutputSession::DeprojectScreenToWorld(const FVector2D& InScreenPosition, FVector& OutWorldPosition, FVector& OutWorldDirection) const
{
	bool bSuccess = false;
...
#if WITH_EDITOR
void URemoteCameraOutputSession::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	FProperty* Property = PropertyChangedEvent.MemberProperty;

	if (Property && PropertyChangedEvent.ChangeType != EPropertyChangeType::Interactive)
	{
		static FName NAME_PortNumber = GET_MEMBER_NAME_CHECKED(URemoteCameraOutputSession, PortNumber);
		//static FName NAME_FromComposureOutputProviderIndex = GET_MEMBER_NAME_CHECKED(URemoteCameraOutputSession, FromComposureOutputProviderIndex);
